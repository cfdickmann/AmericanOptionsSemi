...
double AmericanOption::semi_Basisfunktionen(int zeit, int j, double* x) {
	...
	if (D > 2) {
		...
		if (j < 100) {
			double xx[D];
			for (int d = 0; d < D; ++d)
				xx[d] = (double) j / 100. * 3. * x[d];
			return payoff(xx, zeit);
		}
		j -= 100;

		if (j < 100) {
			return max(y[reihe[0]]- (double)(j)/100.*3,0);
		}
		j -= 100;
		...
		if (j < 100) {
			...
			return max(diff - (double) (j) / 100. * 1. * (double) D * X0[0], 0);
		}
		j -= 100;
		...
		if (j < 100) {
			...
			for (int d = 0; d < D; ++d)
				summe += x[d];
			return max(summe - (double) (j) / 100. * 4. * (double) D * X0[0], 0);
		}
		j -= 100;
		...
		if (j < 100) {
			...
			for (int d = 0; d < D; ++d)
				if(d!=j%3)
					summe += x[d];
			return max(summe - (double) (j) / 100. * 3. * (double) D * X0[0], 0);
		}
		j -= 100;
	}
