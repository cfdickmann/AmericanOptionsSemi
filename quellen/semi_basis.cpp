...
double AmericanOption::semi_Basisfunktionen(int zeit, int j, double* x) {
	if (D == 1)
		return semi_Basisfunktionen1D(zeit, j, x);
	if (D == 2)
		return semi_Basisfunktionen2D(zeit, j, x);
	if (D > 2)
		return semi_BasisfunktionenHigherD(zeit, j, x);
	printf("Error598");
	return -1;
}
...
double AmericanOption::semi_Basisfunktionen2D(int zeit, int j, double* x) {
	...
	if (j == 7)
		return EB.max_call(dt * zeit, T, x, D, Strike, r, delta, sigma[0]);
...
double AmericanOption::semi_BasisfunktionenHigherD(int zeit, int j, double* x) {
	...
	if (j < 1)
		return x[reihe[0]] * x[reihe[0]] * x[reihe[1]];
	j -= 1;

	if (j < 1)
		return x[reihe[1]] * x[reihe[1]] * x[reihe[0]];
	j -= 1;
